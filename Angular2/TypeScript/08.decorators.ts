...
function funDecorator() {
	...
	function log(target: Object, propertyKey: string, descriptor: TypedPropertyDescriptor<any>): any {
		...
		console.log('target:', target) // TestClass
		console.log('propertyKey:', propertyKey) // testMethod
		// 改写testMethod方法，参数不变
		descriptor.value = function (...args: any[]): any {
			console.log('args:', args); // 调用前
			console.log('JSON.stringify(args):', JSON.stringify(args)); // 调用前
	...
	// 下面代码进行测试
	console.log(new TestClass().testMethod('test mothod decorator'));
}
// funDecorator();
...
function classDecorator() {
	...
	class Person { // 这里重复是跟其他文件的Person类重复
		constructor(
	...
	function Component(component) {
		// console.log('component:', component); // @Component后面的参数
		return (target: any) => {
			// console.log('Component-target:', target);  // class Person，这里解析成function
...
function decorator() {
	...
	function Directive() {
		...
		return (target: any) => {
			console.log('directive call'); // 执行早于Component return 的 内容
			return target;
		}
	}
